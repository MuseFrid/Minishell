...
void	create_outfile(int fd[2], int pick, char *file_to_open)
{
	if (fd[OUT] != -2)
		close(fd[OUT]);
	if (pick == D_GREATER)
...
void	open_infile(int *which, int pick, char *file_to_open)
{
	if (*which != -2 && *which != 3)
		close(*which);
	if (pick == D_LOWER)
...
void	redirection_hub(t_arg *redirection, t_data *data, int fd[2])
{
	...
	fd[OUT] = -2;
	which = -2;
	while (redirection && ret_val == 0)
	{
		if (redirection->is_token == D_GREATER || redirection->is_token == GREATER)
			create_outfile(fd, redirection->is_token, redirection->word);
		else
			open_infile(&which, redirection->is_token, redirection->word);
		redirection = redirection->next;
	}
	if (which != -3)
	{
